...
str_EleXpath = "SELECT NAME, XPATH FROM JEE_RESULT_ELEMENT_XPATH_25"
...
def Data_Xpath_Elements():
    # Use the dictionary to fetch XPaths dynamically
    v_AppNo = driver.find_element(By.XPATH, ).text
    v_RollNo = driver.find_element(By.XPATH, xpath_dict.get('v_RollNo')).text
    v_CName = driver.find_element(By.XPATH, xpath_dict.get('v_CName')).text
    v_MName = driver.find_element(By.XPATH, xpath_dict.get('v_MName')).text
    v_FName = driver.find_element(By.XPATH, xpath_dict.get('v_FName')).text
    v_Category = driver.find_element(By.XPATH, xpath_dict.get('v_Category')).text
    v_PwBD = driver.find_element(By.XPATH, xpath_dict.get('v_PwBD')).text
    v_Gender = driver.find_element(By.XPATH, xpath_dict.get('v_Gender')).text
    v_DoB = driver.find_element(By.XPATH, xpath_dict.get('v_DoB')).text
    v_StateofEligibility = driver.find_element(By.XPATH, xpath_dict.get('v_StateofEligibility')).text
    v_Nationality = driver.find_element(By.XPATH, xpath_dict.get('v_Nationality')).text

    v_Physics = driver.find_element(By.XPATH, xpath_dict.get('v_Physics')).text
    v_Chemistry = driver.find_element(By.XPATH, xpath_dict.get('v_Chemistry')).text
    v_Mathematics = driver.find_element(By.XPATH, xpath_dict.get('v_Mathematics')).text
    v_Total = driver.find_element(By.XPATH, xpath_dict.get('v_Total')).text
    v_TotinWords = driver.find_element(By.XPATH, xpath_dict.get('v_TotinWords')).text


    insert_stu_dtls = \
                "INSERT INTO O_JEEMAINS_RESULTS_25_T (ADMNO, APPNO, ROLLNO_SESS1, CANDIDATE_NAME, DOB, GENDER, MOTHER, FATHER, CATEGORY, PWBD, STATEOFELIGIBILITY, NATIONALITY, MATHEMATICS, PHYSICS, CHEMISTRY, TOTAL, TOTAL_IN_WORDS, PROCESS_STATUS, PROCESS_USER) \
    ...
    update_IpStatus = "UPDATE I_JEEMAINS_RESULTS_PWD_25_T SET PROCESS_STATUS = 'D', PROCESS_USER = '"+ str(v_process_user) +"', CREATEDDATE = SYSDATE WHERE APPNO = '"+ str(v_appno) +"'"
...
str_Jeeappno = "SELECT SNO, TRIM(APPNO) APPNO, TRIM(PASSWORD) PASSWORD, ADMNO FROM I_JEEMAINS_RESULTS_PWD_25_T \
            ...
            while attempts < max_attempts and not login_successful:
                ...
                try:
                    ...
                    if v_invalid == "Invalid Application Number/Password":        
                        update_IpStatus = "UPDATE I_JEEMAINS_RESULTS_PWD_25_T SET PROCESS_STATUS = 'Invalid', PROCESS_USER = '"+ str(v_process_user) +"', CREATEDDATE = SYSDATE, ERROR_MESSAGE = '"+ str(v_invalid) +"' WHERE APPNO = '"+ str(v_appno) +"'"
                        cur.execute(update_IpStatus) # Execute an UPDATE statement
                        conn.commit()
                        login_successful=True
                    else:
